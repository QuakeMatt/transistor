...
export interface Process {
    ...
    hasFinished: (time: number) => boolean;
    // getOpacity: (element: HTMLElement, time: number) => number;
    getTransform: (element: HTMLElement, time: number, parent: Rectangle) => Rectangle | null;
...
export function createProcess(
    ...
    console.log('createProcess', {graph, tweens});
    ...
    const linearEasing = createBezierEasing(0.0, 0.0, 1.0, 1.0);
    ...
    function hasFinished(time: number): boolean {
        return time > endTime;
    }
    ...
    function getTransform(element: HTMLElement, time: number, parent: Rectangle): Rectangle | null {
        ...
        // console.log({element, time, progress, invgress});

        const matt = element.classList.contains('matt') ? 2.0 : 1.0;

        let tx = (start.rectangle.x * matt - final.rectangle.x) * invgress;
